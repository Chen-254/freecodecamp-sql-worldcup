...
echo -e "$($PSQL "SELECT na.name FROM games INNER JOIN teams AS na ON games.winner_id = na.team_id WHERE year=2014 AND round = 'Eighth-Final' UNION SELECT nb.name FROM games INNER JOIN teams AS nb ON games.opponent_id = nb.team_id WHERE year=2014 AND round = 'Eighth-Final'")"
